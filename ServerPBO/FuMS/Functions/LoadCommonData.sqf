...
diag_log format ["<FuMS> LoadCommonData: Preparing FuMS common data."];
//BaseServer Init
_hold = [] execVM "\FuMS\Themes\BaseServer.sqf";
if (isNil "_hold") exitWith { diag_log format ["<FuMS> LoadCommonData: ERROR in BaseServer.sqf format."]; FuMS_isStable=false;};
waitUntil { ScriptDone _hold};
_hold = [] execVM "\FuMS\Themes\BaseSoldier.sqf";
if (isNil "_hold") exitWith { diag_log format ["<FuMS> LoadCommonData: ERROR in BaseLoot.sqf and/or BaseSoldier.sqf"];FuMS_isStable=false;};
waitUntil { ScriptDone _hold};
...
diag_log format ["<FuMS> LoadCommonData: ActiveThemes: %1",FuMS_ActiveThemes];
diag_log format ["<FuMS> LoadCommonData: ActiveThemesHC: %1",FuMS_ActiveThemesHC];
...
_abort = false;
{
    ...
    if (isNil "_hold") then { diag_log format ["<FuMS> LoadCommonData: ERROR in %1\ThemeData.sqf format in theme #%1",_x];}
    else
    {
        waitUntil { scriptDone _hold}; 
        ...
        if (!(((FuMS_THEMEDATA select _themeNumber) select 0) select 3) ) then //if theme using its own LootData
        {
            ...
            { 
                diag_log format ["************************************************************************"];
                diag_log format ["<FuMS> LoadCommonData: %1/ThemeData GlobalLootData 'true' and error in %1/LootData.sqf",_x];
        ...
        if (!(((FuMS_THEMEDATA select _themeNumber) select 0) select 4 )) then // if theme using its own soldierData
        {
            ...
            { 
                diag_log format ["************************************************************************"];
                diag_log format ["<FuMS> LoadCommonData: %1/ThemeData GlobalSoldierData 'true' and error in %1/SoldierData.sqf",_x];
                diag_log format ["<FuMS> LoadCommonData: Theme offline!"];
                diag_log format ["************************************************************************"];
                _abort = true;
                FuMS_isStable=false;
        ...
        {
            diag_log format ["<FuMS> LoadCommonData: %1:%2",_themeNumber,_x];
...
if (_abort) exitWith { diag_log format ["<FuMS> LoadCommonData: Critical Error: Exiting!"];FuMS_isStable=false;};
FuMS_BaseTHEMEDATA = +FuMS_THEMEDATA;

diag_log format ["<FuMS> LoadCommonData: THEMEDATA: %1", FuMS_BaseTHEMEDATA];
...
if (isNil "_hold") exitWith { diag_log format ["<FuMS> LoadCommonData: ERROR in GlobalLootData.sqf: Exiting!"];FuMS_isStable=false;};
waitUntil { scriptDone _hold};
...
if (isNil "_hold") exitWith { diag_log format ["<FuMS> LoadCommonData: ERROR in GlobalSoldierData.sqf: Exiting!"];FuMS_isStable=false;};
waitUntil { scriptDone _hold};
...
diag_log format ["<FuMS> LoadCommonData Complete ##"];
