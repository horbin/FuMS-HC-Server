...
//  Encounter Center
//  Radius
private ["_missionName","_themeIndex","_msnText","_ogjstr","_arrayOfData","_eCenter","_ZupaTrackerVar"];
...
_ZupaTrackerVar = format ["FuMS_Zupa_%1%2",_missionName,_themeIndex];
...
// for each ZupaCapture data element [offset], radius, time, "name"]
{
    ...
    _mkrName = format ["FuMS_%1%2",_missionName,_capturePosition];
    ...
    [_capturePosition, _ZCP_CapRadius, _ZCP_CapTime, _capPointName, _missionName, _mkrName, _dotName, _ZupaTrackerVar  ] spawn
    {
...
[ _ZupaTrackerVar, _themeIndex, _missionName, count _arrayOfData] spawn
{
    private ["_done","_count","_total","_ogjstr","_msnText","_themeIndex","_ZupaTrackerVar","_missionName"];
    _ZupaTrackerVar = [];
    
    //spawn process above may take a few cycles to complete, so need to wait until all cap points are fully constructed!
    while {count _ZupaTrackerVar != _this select 3} do
    {
